...
namespace MyDotnet.Helper
{
    ...
    public static class CodeHelper
    {
        ...
        private static readonly int _fontHeight = 24;

        /// <summary>
        /// 泡泡数量
        /// </summary>
        private static int _circleCount = 10;
        /// <summary>
        /// 泡泡半径范围
        /// </summary>
        private static readonly int _miniCircleR = 2;
        private static readonly int _maxCircleR = 8;
        ...
        /// 字体池
        /// </summary>
        private static List<Font> _fontArr;
        ...
        public static byte[] GetBubbleCodeByte(string text)
        {
            ...
            var tempCircleCount = _circleCount;
            if (_random.Next(0, 6) == 3)
            {
                colorCircleHex = "#FFFFFF";//白色
                tempCircleCount = (int)(_circleCount * 2.65);
            }
            var white = new Rgba32(255, 255, 255); // 创建白色

            img.Mutate(ctx => ctx
                    .Fill(white)
                    .DrawingCnText(_imageWidth, _imageHeight, text, ParseHexColor(colorTextHex), _fontArr[_random.Next(0, _fontArr.Count)])
                    .DrawingCircles(_imageWidth, _imageHeight, tempCircleCount, _miniCircleR, _maxCircleR, ParseHexColor(colorCircleHex))
                );
        ...
        /// <returns>验证码图片字节数组</returns>
        public static byte[] GetEnDigitalCodeByte(string text)
        {
            using Image<Rgba32> img = new(_imageWidth, _imageHeight);
        ...
        private static void InitFonts(int fontSize)
        {
            ...
            var sourceDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ComFile", "fonts");
            if (Directory.Exists(sourceDir))
            {
                var fontFiles = Directory.GetFiles(sourceDir, "*.ttf");
                var data = from fontFile in fontFiles let fontCollection = new FontCollection() select new Font(fontCollection.Add(fontFile), fontSize);
                list.AddRange(data);  
            }
            else
            {
