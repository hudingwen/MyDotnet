                        ...
                        if (data.success)
                        {
                            ...
                            if (data.data.followedDeviceGlucoseDataPO.glucoseInfos != null && data.data.followedDeviceGlucoseDataPO.glucoseInfos.Count > 0 && data.data.followedDeviceGlucoseDataPO.time == data.data.followedDeviceGlucoseDataPO.glucoseInfos[0].time)
                            {
                                ...
                                if (pushData.Count > 0)
                                {
                                    ...
                                    nextTime = pushData[pushData.Count - 1].time.AddMinutes(5).AddSeconds(10);
                                    isTouchTime = true;
                                }
                            }
                            else
                            {
                                ...
                                if (data.data.followedDeviceGlucoseDataPO.time > user.refreshTime)
                                {
                                    ...
                                    await _nightscoutGuardService.pushBlood(user, send);
                                    nextTime = DateTimeOffset.FromUnixTimeMilliseconds(send[send.Count - 1].date).UtcDateTime.ToLocalTime().AddMinutes(5).AddSeconds(10);
                                    isTouchTime = true;
                                }
                            }
                    ...
                    else if ("300".Equals(account.guardType))
                    {
                        ...
                        if ("100000".Equals(data.info?.code))
                        {
                            ...
                            if (pushData.Count > 0)
                            {
                                ...
                                await _nightscoutGuardService.pushBlood(user, send);
                                nextTime = pushData[pushData.Count - 1].deviceTime.AddMinutes(5).AddSeconds(10);
                                isTouchTime = true;
                            }
                    ...
                    else if ("500".Equals(account.guardType))
                    {
                        ...
                        if (data.state == 1)
                        {
                            ...
                            if (pushData.Count > 0)
                            {
                                ...
                                await _nightscoutGuardService.pushBlood(user, send);
                                nextTime = pushData[pushData.Count - 1].timeFormat.AddMinutes(5).AddSeconds(10);
                                isTouchTime = true;
                            }
                        ...
                        if (data.success)
                        {
                            ...
                            if (data.data.deviceInfo.glucoseInfos != null && data.data.deviceInfo.glucoseInfos.Count > 0 && data.data.deviceInfo.latestTimeFormat == data.data.deviceInfo.glucoseInfos[0].time)
                            {
                                ...
                                if (pushData.Count > 0)
                                {
                                    ...
                                    await _nightscoutGuardService.pushBlood(user, send);
                                    nextTime = pushData[pushData.Count - 1].time.AddMinutes(5).AddSeconds(45);
                                    isTouchTime = true;
                                }
                            }
                            else
                            {
                                ...
                                if (data.data.deviceInfo.latestTimeFormat > user.refreshTime)
                                {
                                    ...
                                    await _nightscoutGuardService.pushBlood(user, send);
                                    nextTime = DateTimeOffset.FromUnixTimeMilliseconds(send[send.Count - 1].date).UtcDateTime.ToLocalTime().AddMinutes(5).AddSeconds(45);
                                    isTouchTime = true;
                                }
                            }
                    ...
                    else if ("600".Equals(account.guardType))
                    {
                        ...
                        if (data.status == 0)
                        {
                            ...
                            if(data.data.connection.glucoseItem.time > user.refreshTime)
                            {
                                ...
                                await _nightscoutGuardService.pushBlood(user, send);
                                nextTime = data.data.connection.glucoseItem.time.AddMinutes(5).AddSeconds(10);
                                isTouchTime = true;
                            }
