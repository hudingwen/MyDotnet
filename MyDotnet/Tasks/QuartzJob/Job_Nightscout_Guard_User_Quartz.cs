                        ...
                        if (data.success)
                        {
                            ...
                            if (data.data.followedDeviceGlucoseDataPO.glucoseInfos != null && data.data.followedDeviceGlucoseDataPO.glucoseInfos.Count > 0)
                            {
                                ...
                                if (pushData.Count > 0)
                                {
                                    ...
                                    nextTime = pushData[pushData.Count - 1].time.AddMinutes(5).AddSeconds(1);
                                    isTouchTime = true;
                                }
                            }
                            else
                            {
                                ...
                                if (data.data.followedDeviceGlucoseDataPO.time > user.refreshTime)
                                {
                                    ...
                                    await _nightscoutGuardService.pushBlood(user, send);
                                    nextTime = DateTimeOffset.FromUnixTimeMilliseconds(send[send.Count - 1].date).UtcDateTime.ToLocalTime().AddMinutes(5).AddSeconds(1);
                                    isTouchTime = true;
                                }
                            }
