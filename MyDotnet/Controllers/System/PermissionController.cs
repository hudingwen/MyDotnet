...
namespace MyDotnet.Controllers.System
{
    ...
    public class PermissionController : BaseApiController
    {
        ...
        public async Task<MessageModel<List<Permission>>> MigratePermission(string action = "", string token = "", string gatewayPrefix = "", string swaggerDomain = "", string controllerName = "", long pid = 0, bool isAction = false)
        {
            ...
            var Configuration = swaggerDomain.IsNotEmptyOrNull() ? swaggerDomain : ConfigHelper.GetValue("SystemCfg:Domain");
            ...
            var modulesList = (await _moduleServices.Dal.Query(d => d.IsDeleted == false && d.LinkUrl != null)).Select(d => d.LinkUrl.ToLower()).ToList();
            ...
            if (isAction)
            {
                foreach (var item in permissions)
                {
                    List<Modules> modules = await _moduleServices.Dal.Query(d => d.LinkUrl != null && d.LinkUrl.ToLower() == item.Module.LinkUrl);
                    if (!modules.Any())
                    {
                        var mid = await _moduleServices.Dal.Add(item.Module);
                        if (mid > 0)
                        {
                            item.Mid = mid;
                            var permissionid = await _permissionServices.Dal.Add(item);
                        }
