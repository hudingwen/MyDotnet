...
namespace MyDotnet.Services.System
{
    ...
    /// TasksLogServices
    /// </summary>
    public partial class TasksLogServices : BaseServices<TasksLog>
    {
        public TasksLogServices(BaseRepository<TasksLog> baseRepository) : base(baseRepository)
        {
        }
        ...
        public async Task<PageModel<TasksLog>> GetTaskLogs(long jobId, int page, int intPageSize, DateTime? runTime, DateTime? endTime)
        {
            ...
            var data = await Dal.Db.Queryable<TasksLog>()
        ...
        public async Task<object> GetTaskOverview(long jobId, DateTime? runTime, DateTime? endTime, string type)
        {
            ...
            if ("year".Equals(type))
            {
                ...
                var queryableLeft = Dal.Db.Reportable(dayArray).ToQueryable<DateTime>();
                var queryableRight = Dal.Db.Queryable<TasksLog>().Where((x) => x.RunTime.Year >= runTime.Value.Year && x.RunTime.Year <= endTime.Value.Year); ; ; //声名表

                var list = Dal.Db.Queryable(queryableLeft, queryableRight, JoinType.Left,
            ...
            {
                //按月
                var queryableLeft = Dal.Db.Reportable(ReportableDateType.MonthsInLast1years).ToQueryable<DateTime>(); //生成月份 //ReportableDateType.MonthsInLast1yea 表式近一年月份 并且queryable之后还能在where过滤
                var queryableRight = Dal.Db.Queryable<TasksLog>().Where((x) => x.RunTime.Year == runTime.Value.Year); //声名表
            ...
            else if ("day".Equals(type))
            {
                ...
                var queryableRight = Dal.Db.Queryable<TasksLog>().Where((x) => x.RunTime >= star && x.RunTime <= end); ; ; //声名表
            ...
            else if ("hour".Equals(type))
            {
                ...
                var queryableLeft = Dal.Db.Reportable(dayArray).ToQueryable<DateTime>();
                var queryableRight = Dal.Db.Queryable<TasksLog>().Where((x) => x.RunTime >= runTime.Value.Date && x.RunTime <= runTime.Value.Date.AddDays(1).AddMilliseconds(-1)); //声名表
